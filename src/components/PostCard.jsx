...
const PostCard = ({userId, status, uImg, uName, postContent, deletePost, datetime, postId}) => {
  return ( 
    <div className="w-full py-5 px-3 flex flex-col items-start text-sm rounded-md shadow-md hover:bg-slate-200/50 duration-200 transition-all cursor-pointer">
        ...
        
        {
          status ? <div className="w-full flex gap-2 justify-between items-center pt-5 px-10 text-xs">
              <span className="flex justify-center items-center gap-1 hover:text-green-700"><FaHeart className=" text-sm"/> Like</span>
              <span className="flex justify-center items-center gap-1 hover:text-blue-600"><FaBookmark className=" text-sm"/> Bookmark</span>
              <span className="flex justify-center items-center gap-1 hover:text-red-700"><FaTrash
              //  onClick={deletePost} ÷\
